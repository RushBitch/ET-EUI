...
namespace ET
{
    public partial class BattlefieldMapConfig
    {
        ...
        public List<int> GetHeroSpawnIndexs()
        {
            List<int> list = new List<int>();
            for (int i = 0; i < this.battlefieldMap.Length; i++)
            {
                for (int j = 0; j < this.battlefieldMap[i].Length; j++)
                {
                    if (this.battlefieldMap[i][j] == 1)
                    {
                        list.Add(i * this.battlefieldMap[i].Length + j);
                    }
                }
            }
        ...
        public List<int> GetHeroSpawnIndexsInPvp(int index)
        {
            List<int> list = new List<int>();
            for (int i = 0; i < this.battlefieldMap.Length; i++)
            {
                for (int j = 0; j < this.battlefieldMap[i].Length; j++)
                {
                    if (this.battlefieldMap[i][j] == 1)
                    {
                        int count = i * this.battlefieldMap[i].Length + j;
                        if (index == 0 && count < 35)
                        {
                            list.Add(count);
                        }
